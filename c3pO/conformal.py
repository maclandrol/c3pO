...
from c3pO.utils import coverage_size, sort_sum
...
def validate_model(val_loader, model):
    ...
    with torch.no_grad():
        ...
        for i, (x, target) in enumerate(val_loader):
            ...
            # Update meters
            coverage.append((cvg, x.shape[0]))
            size.append((sz, x.shape[0]))

    cvg_avg = np.mean([x * y for x, y in coverage])
    avg_sz = np.mean([x * y for x, y in size])
    return cvg_avg, avg_sz
    ...
    def __init__(
        ...
        if not naive and not LAC:
            ...
            scores_cal = 1 - np.array(
                [
                    np.sort(torch.softmax(calib_loader.dataset[i][0] / self.T.item(), dim=0))[::-1][
                        gt_locs_cal[i]
                    ]
...
def pick_lamda_size(model, paramtune_loader, alpha, kreg, randomized, allow_zero_sets):
    ...
    for temp_lam in [
        ...
        cvg_avg, sz_avg = validate_model(paramtune_loader, conformal_model)
        if sz_avg < best_size:
            best_size = sz_avg
            lamda_star = temp_lam
    return lamda_star
